...
public:
    ...
    bool fitLine(const std::vector<cv::Point>& pts, cv::Point2d& pt1, cv::Point2d& pt2, cv::Point2d& pt3, cv::Point2d& pt4);
    bool fitCircle(const std::vector<cv::Point>& pts, cv::Point2d& center, double& radius);
    bool fitEllipse(const std::vector<cv::Point>& pts, cv::Point2d& m, cv::Point2d& v, float& theta);
